(() => {
    ...
    function initSliders() {
        if (document.querySelector(".transformation") && document.querySelector(".text-transformation")) {
            const imageSwiper = new swiper_core_Swiper(".transformation", {
            ...
            const textSwiper = new swiper_core_Swiper(".text-transformation", {
        ...
        } else console.error("Не найдены необходимые элементы для инициализации слайдеров.");
    }
    ...
    const typewriterFunc = () => {
        ...
        const typing = typewtiterElement => {
            const delay = typewtiterElement.dataset.delay || 2e3;
            const startDelay = typewtiterElement.dataset.startDelay || 0;
            typewtiterElement.dataset.removeDelay;
            typewtiterElement.dataset.addSpeed;
    ...
    const showTexts = () => {
        ...
        if (showTextsArray.length > 0) {
            ...
            if (showTextsRegular.length > 0) initshowTexts(showTextsRegular);
            ...
            function initShowTextBody(showTextsBlock, hideShowTextBody = true) {
                ...
                if (showTextTitles.length > 0) showTextTitles.forEach((showTextTitle => {
                    console.log("lena", showTextTitle);
                    if (hideShowTextBody) {
                        showTextTitle.removeAttribute("tabindex");
                        if (!showTextTitle.classList.contains("_active")) showTextTitle.previousElementSibling.hidden = true;
                    } else {
            ...
            function setShowTextAction(e) {
                ...
                if (el.hasAttribute("data-showtext") || el.closest("[data-showtext]")) {
                    ...
                    if (!showtextTitle.classList.contains("_active")) showtextTitle.innerHTML = "Свернуть<span></span>"; else showtextTitle.innerHTML = "Читать далее<span></span>";
                    ...
                    if (!showtextsBlock.querySelectorAll("_slide").length) {
                        if (oneShowtext && !showtextTitle.classList.contains("_active")) hideShowtextsBody(showtextsBlock);
            ...
            function hideShowtextsBody(showtextsBlock) {
                ...
                if (showtextActiveTitle) _slideUp(showtextActiveTitle.previousElementSibling, 500);
            }
        }
    ...
    const phoneInput = () => {
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            let onPhoneInput = function(e) {
                let input = e.target, inputNumbersValue = getInputNunbersValue(input), formatedInputValue = "", selectionStart = input.selectionStart;
                ...
                if ([ "3", "8", "0" ].indexOf(inputNumbersValue[0]) > -1) {
                    ...
                    if (firstSymbols === "+38") {
                        if (inputNumbersValue.length > 1) formatedInputValue += " (" + inputNumbersValue.substring(2, 5);
                        if (inputNumbersValue.length >= 6) formatedInputValue += ") " + inputNumbersValue.substring(5, 8);
                        if (inputNumbersValue.length >= 9) formatedInputValue += "-" + inputNumbersValue.substring(8, 10);
                        if (inputNumbersValue.length >= 11) formatedInputValue += "-" + inputNumbersValue.substring(10, 12);
                    } else {
                        if (inputNumbersValue.length > 1) formatedInputValue += " (" + inputNumbersValue.substring(1, 4);
                        if (inputNumbersValue.length >= 5) formatedInputValue += ") " + inputNumbersValue.substring(4, 7);
                        if (inputNumbersValue.length >= 8) formatedInputValue += "-" + inputNumbersValue.substring(7, 9);
                        if (inputNumbersValue.length >= 10) formatedInputValue += "-" + inputNumbersValue.substring(9, 11);
                    }
                } else formatedInputValue = "+" + inputNumbersValue.substring(0, 16);
                input.value = formatedInputValue;
            };
            let onPhoneKeyDown = function(e) {
                let input = e.target;
                if (e.keyCode == 8 && getInputNunbersValue(input).length == 2) input.value = "";
            };
            let onPhonePaste = function(e) {
                e.clipboardData || window.clipboardData;
                let input = e.target, inputNumbersValue = getInputNunbersValue(input);
                if (pasted) {
    ...
    const hoverRelation = () => {
        ...
        if (linksArray.length > 0) for (let i = 0; i < linksArray.length; i++) {
